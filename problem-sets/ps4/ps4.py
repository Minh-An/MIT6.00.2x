...
def generate_models(x, y, degs):
    ...
    yVals = np.array(y)
    for d in degs:
        coefficient = np.polyfit(xVals, yVals, d)
...
def r_squared(y, estimated):
    ...
    """
    mean = sum(y) / len(y)
    modelDiff = sum([(y[i] - estimated[i])**2 for i in range(len(y))]) 
    variance = sum([(i - mean)**2 for i in y])
    return 1- (modelDiff/variance)
