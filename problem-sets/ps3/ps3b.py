...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for j in range(300):
            totalPop[j] += patient.update()
    
    pylab.plot([i for i in range(1,301)], [i/numTrials for i in totalPop], label = "SimpleVirus")
        ...
        if canReproduce:
            if random.random() <= self.maxBirthProb * (1 - popDensity):
                ...
                for resistance in self.getResistances():
                    if not random.random() <= 1-self.getMutProb():
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in self.viruses:
            try:
                v = virus.reproduce(popDensity, self.getPrescriptions())
                if v != None:    
                    newViruses.append(v)
            except NoChildException:
                pass
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    totalPop = [0 for i in range(300)]
    guttagonolResistant = [0 for i in range(300)]
    for i in range(numTrials):
        ...
        for j in range(300):
            ...
            guttagonolResistant[j] += patient.getResistPop(['guttagonol'])
    
    pylab.plot([i for i in range(1,301)], [i/numTrials for i in totalPop], label = "Total Virus Population")
    pylab.plot([i for i in range(1,301)], [i/numTrials for i in guttagonolResistant], label = "Guttagonol Resistant Population")
    pylab.title('Resistant Virus Simulation With Drugs')
    pylab.xlabel('Time Steps')
    pylab.ylabel('Average Virus Population')
    pylab.legend(loc = 'upper left')
    pylab.show()
